...
### 🎯 新增功能
- 📚 **工具书系统**: 强大的知识库管理功能
  - 关键词触发机制：对话中出现关键词自动注入内容
  - DOCX文档导入：支持从Word文档批量导入工具书
  - AI图片识别：导入时可选择使用AI识别文档中的图片
  - 资源管理：支持为工具书添加图片、文件等附件
  - 位置控制：支持前置/后置注入，可调优先级
  - 概率触发：可设置触发概率和扫描深度
  - 📸 **AI图片推送功能**: AI主动展示工具书图片
    - 全局开关控制总功能
    - 单个工具书独立开关
    - AI自动识别并推送相关图片
    - 图片以HTML格式优雅展示
...
### 分支管理
- **main分支**: v2.5.0及后续扩展版本（包含文档解析等新功能）
...
│   ├── regex-manager.js    # 正则表达式管理
│   ├── regex-engine.js     # 正则替换引擎
│   ├── frontend-decorator.js # 前端文本修饰器
│   ├── html-renderer.js    # HTML渲染器
│   └── text-decorator.js   # 文本装饰工具
│
├── 🎨 样式文件
│   ├── world.css          # 世界书&工具书样式
│   ├── regex.css          # 正则管理器样式
│   └── text-decorator.css # 文本修饰器样式
│
├── 🛠️ 后端模块
│   ├── document_parser.py  # 文档解析器
│   └── docx_extract.py     # DOCX提取（含AI识图）
│
├── 🚀 启动脚本
...
└── 💾 数据目录
    └── data/
        ...
        ├── worlds/        # 世界书 (JSON)
        ├── toolbooks/     # 工具书数据
        │   ├── toolbooks_index.json  # 工具书索引
        │   ├── active_toolbooks.json # 激活状态
        │   ├── *.txt      # 工具书内容
        │   └── */         # 工具书资源文件夹
        ├── presets/       # 预设配置 (JSON)
        ├── config.json    # 全局配置
        ├── frontend_decorator.json # 前端修饰器配置
        ├── regex_scripts.json # 正则脚本配置
        └── user_persona.json  # 用户身份信息
```
...
- **存储**: LocalStorage + 服务器文件系统
- **通信**: Fetch API + EventSource (SSE)
- **渲染**: 自定义HTML渲染器 + iframe沙箱
...
- **请求**: Requests
- **数据**: JSON/JSONL 文件存储
- **文档解析**: PyPDF2 / Unstructured / python-docx
- **图片处理**: Pillow

### 核心模块
- **工具书系统**: 关键词触发 + 资源管理
- **预设管理**: SillyTavern兼容格式
- **正则引擎**: 全局/角色脚本支持
- **文本修饰**: 引号/动作装饰器
- **HTML渲染**: 深度控制 + 安全沙箱
